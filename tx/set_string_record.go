...
// SetStringRecord 写入字符串：<SETSTRING, 0, junk, 33, 12, joe>
...
func (s *SetStringRecord) ToString() string {
	return fmt.Sprintf("<SETSTRING %d %d %d %s>", s.tx_num, s.blk.Number(), s.offset, s.val)
}
...
func WriteSetStringLog(log_manager *lg.LogManager, tx_num uint64,
	...
	*/
	...
	opos := bpos + UINT64_LENGTH
	vpos := opos + UINT64_LENGTH
	rec_len := vpos + p.MaxLengthForString(val)
