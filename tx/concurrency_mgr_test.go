...
func TestCurrencyManager(_ *testing.T) {
	...
	//tx.NewTransaction(file_manager, log_manager, buffer_manager)
	go func() {
		txA := NewTransaction(file_manager, log_manager, buffer_manager)
	...
	go func() {
		...
		txB := NewTransaction(file_manager, log_manager, buffer_manager)
		blk1 := fm.NewBlockId("testfile", 1)
		blk2 := fm.NewBlockId("testfile", 2)
		txB.Pin(blk1)
		txB.Pin(blk2)
	...
	go func() {
		time.Sleep(2 * time.Second)
		txC := NewTransaction(file_manager, log_manager, buffer_manager)
		blk1 := fm.NewBlockId("testfile", 1)
		blk2 := fm.NewBlockId("testfile", 2)
		txC.Pin(blk1)
		txC.Pin(blk2)
